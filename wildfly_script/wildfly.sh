...
WILDFLY_ADMIN_USER="wildfly"
WILDFLY_ADMIN_PASS="wildfly"
...
if [[ $EUID -ne 0 ]]; then
      ...
      if [ ! -e "$WILDFLY_ARCHIVE_NAME" ]; then
		  ...
		  if [ $? -ne 0 ]; then
			    ...
			    ${WILDFLY_DIR}/bin/add-user.sh -u "${WILDFLY_ADMIN_USER}" -p "${WILDFLY_ADMIN_PASS}"
			     
			    OPTS1='JAVA_OPTS="$JAVA_OPTS -Djboss.bind.address.management=0.0.0.0 -Djboss.bind.address=0.0.0.0 -Djboss.http.port=8080"'
			    grep "${OPTS1}" ${WILDFLY_DIR}/bin/standalone.conf || echo "${OPTS1}" >> ${WILDFLY_DIR}/bin/standalone.conf
